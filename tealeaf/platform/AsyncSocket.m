...
- (NSUInteger)readLengthForTerm
{
	...
	const char *termBuf = (const char*)[term bytes];
	
	while (i < bytesDone)
	{
		char *subbuf = buf + startOffset + i;
...
- (NSUInteger)readLengthForTermWithPreBuffer:(NSData *)preBuffer found:(BOOL *)foundPtr
{
	...
	char *buf = (char*)[buffer mutableBytes] + startOffset + bytesDone - bufLen;
	
	NSUInteger preLen = termLength - bufLen;
	char *pre = (char *)[preBuffer bytes];
	...
	for (i = 0; i < loopCount; i++)
	{
		...
		else
		{
			...
			{
				NSUInteger preOffset = pre - (char*)[preBuffer bytes]; // pointer arithmetic
...
- (NSInteger)searchForTermAfterPreBuffering:(NSUInteger)numBytes
{
	...
	{
		char *subBuffer = (char*)[buffer mutableBytes] + startOffset + i;
